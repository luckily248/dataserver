...
func (this *AddressResBody) Tablename() string {
	return "addressResBody"
}

func (this *AddressResBody) init() {
	this.BaseDBmodel.init()
	this.c = this.db.C(this.Tablename())
}

func GetOneData(cityid string) (content *AddressResBody, err error) {
	content=&AddressResBody{}
	content.init()
...
func UpsertData(content *AddressResBody)error{
	content.init()
	defer content.session.Close()
